...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image my-3" src=${image}></img>
      ...
      <p><b>Rating: </b>${product.rating.rate}</span></p>
      <p><b>Total Rating: </b>${product.rating.count}</p>
      <h2 class="mb-4 color-primary"><b>Price: $${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now button-primary"><i class="fas fa-cart-plus"></i> Add to cart</button>
      <button id="details-btn" class="button-secondary"><i class="fas fa-angle-double-right"></i> Details</button></div>
      `;
